...
void init_rectification_map(string param_file_path) 
{
  ...
  double k1[3][3]={{286.063110351562, 0.0, 421.770111083984},{0.0, 286.130096435547, 402.908294677734},{0.0, 0.0, 1.0 }};
  cv::Mat K1(3,3, CV_64F, k1);

  double k2[3][3]={{285.918212890625, 0.0, 419.524505615234},{  0.0, 285.952606201172, 404.546508789062},{0.0, 0.0, 1.0}};
  cv::Mat K2(3,3, CV_64F, k2);

  double k3[1][4]={{-0.00424315920099616, 0.0391328111290932, -0.0373711809515953 ,0.00625591585412621}};
  cv::Mat D1(1,4, CV_64F, k3);

  double k4[1][4]={{-0.00490465201437473, 0.0416384600102901 ,-0.0396286398172379 ,0.00687009189277887}};
  cv::Mat D2(1,4, CV_64F, k4);
 
  double k5[3][3]={{0.999984 , -0.000969337 , -0.00550959},{ 0.000974116,  0.999999,  0.000864823},{  0.00550875, -0.000870177,  0.999984 }};
  cv::Mat R(3,3, CV_64F, k5);

 

  T = { -0.06386798620224,  0.000186239456525072,  -8.98785219760612e-05 };
